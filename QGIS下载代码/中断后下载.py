...
def download_raster_for_polygons(data, output_dir):
    # 获取已有文件的索引集合，以便快速跳过
    existing_files = {int(f.split(".")[0]) for f in os.listdir(output_dir) if f.endswith(".tif")}

    for i, geom in enumerate(data['geometry']):
        # 检查当前多边形是否已下载过
        if i in existing_files:
            print(f"文件已存在，跳过多边形 {i}: {os.path.join(output_dir, f'{i}.tif')}")
            continue
        ...
        if isinstance(geom, Polygon):
            ...
            polygons = list(geom.geoms)  # 从 MultiPolygon 获取所有 Polygon
