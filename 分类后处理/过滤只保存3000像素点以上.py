...
def remove_noise(image_path, output_path):
    ...
    # 计算过滤后图像中非零像素的数量
    pixel_count = np.count_nonzero(output_image)

    # 如果非零像素点数量小于阈值，则不保存图像
    if pixel_count < MIN_PIXELS:
        print(f"图像 {os.path.basename(image_path)} 过滤后像素点数量为 {pixel_count}，未达到保存阈值，跳过保存。")
        return
...
# 调用函数处理文件夹中的所有图片
input_folder = r"I:\APV\7去噪结果\去噪图像"
output_folder = r"I:\APV\7去噪结果\去噪图像res"
